...
def calculate_jacobians(h, clean_logits, classifier, fc_mask_size, consistency_criterion, reset_grad_fn, feat_forward2):
    ...
    h_logits = classifier(feat_forward2(cnn_mask * h))
    discrepancy = consistency_criterion(h_logits, clean_logits)
    discrepancy.backward()

    reset_grad_fn()
    return cnn_mask.grad.clone(), h_logits
