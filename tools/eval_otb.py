...
def eval_auc(dataset='OTB2015', result_path='./test/', tracker_reg='S*', start=0, end=1e6):
    if 'UAV' in dataset:
        base_path = '/home/zhbli/Dataset/data1/UAV123/data_seq/UAV20L'
        seqs = os.listdir(base_path)
    else:
        if dataset == 'OTB2013':
            list_path = '/data/zpzhang/dataset/OTB2013.json'
        else:
    ...
    UAV20L = ['bird1', 'car1', 'car6', 'car9', 'group2', 'person14', 'person19', 'person2', 'person5', 'uav1', 'bike1',
              'car16', 'car3', 'car8', 'group1', 'group3', 'person17', 'person20', 'person4', 'person7']
    ...
    for i in range(n_seq):
        seq = seqs[i]
        if 'UAV' in dataset:
            gt_path = join(base_path, '../../anno', dataset, seq + '.txt')
            gt_rect = np.loadtxt(gt_path, delimiter=',').astype(np.float64)
        else:
...
if __name__ == "__main__":
    if len(sys.argv) < 5:
        print('python eval_otb.py OTB2013 ./test SiamRPN* 0 10')
        exit()
    dataset = sys.argv[1]
